import { deterministicLayout, normalizeViewport } from './graph-layout.js';
...
export class GraphCanvas {
  ...
  setProjection(projection, layout) {
    ...
    this.selection = new Set((layout?.selection || []).filter(id => projection.nodes.some(node => node.id === id)));
  ...
  restoreLayout(layout) {
    ...
    this.viewport = normalizeViewport(layout.viewport);
    this.selection = new Set((layout.selection || []).filter(id => ids.has(id)));
  ...
  visibleNodeIds() {
    ...
    for (const node of this.projection.nodes) {
      ...
      if (point && point.x + 260 >= left && point.x <= right && point.y + 140 >= top && point.y <= bottom)
        visible.add(node.id);
    }
    return visible;
  }
  ...
  nodeElement(node) {
    ...
    card.setAttribute('aria-label', node.title + ', ' + node.kind + ', YAML ' + (node.yamlPath || 'synthetic'));
    ...
    for (const issue of this.projection.diagnostics || [])
      if (issue.nodeId === node.id || issue.yamlPath && (issue.yamlPath === node.yamlPath || issue.yamlPath.startsWith(node.yamlPath + '/')))
        badgeValues.add(String(issue.severity || 'diagnostic').toLowerCase());
    ...
    const action = (label, type) => {
      ...
      button.addEventListener('click', event => { event.stopPropagation(); this.options.onNodeAction?.({ type, node }); });
    ...
    for (const pin of node.pins || []) (pin.direction === 'input' ? inputs : outputs).append(this.pinElement(pin));
  ...
  groupElement(group) {
    ...
    const maxX = Math.max(...points.map(point => point.x + 260)) + 24, maxY = Math.max(...points.map(point => point.y + 120)) + 24;
  ...
  activatePin(pin, button) {
    ...
    if (!this.pendingPin) {
      if (pin.direction !== 'output') {
        this.options.onConnectionError?.('Choose an output pin first, then a compatible input pin.'); return;
      }
      this.pendingPin = pin;
      button.setAttribute('aria-pressed', 'true');
      this.options.onConnectionPending?.(pin);
      return;
    }
    const source = this.pendingPin;
    this.cancelPinConnection();
    if (source.id === pin.id) return;
    this.connectPins(source, pin);
  }

  startPinConnection(event, pin, button) {
    if (event.pointerType !== 'touch' && event.button !== 0 || !this.projection?.editable || pin.direction !== 'output') return;
    event.stopPropagation();
    ...
    const move = current => {
      ...
      this.drawConnectionPreview(pin, current.clientX, current.clientY);
      this.highlightCompatiblePins(pin);
    };
    const end = current => {
      ...
      if (target) this.connectPins(pin, target);
      else this.options.onPalette?.({ clientX: current.clientX, clientY: current.clientY, sourcePin: pin });
    };
  ...
  connectPins(source, target) {
    ...
    for (const edge of this.projection.resourceKind === 'behavior' ? [] : check.replace || []) {
      ...
      operations.push({ type: 'DISCONNECT', yamlPath: edge.sourceYamlPath,
        sourcePinId: edge.sourcePinId, targetPinId: edge.targetPinId });
    }
  ...
  drawConnectionPreview(source, clientX, clientY) {
    const owner = this.positions[source.nodeId]; if (!owner) return;
    if (!this.previewPath) {
    ...
    const x1 = owner.x + 260, y1 = owner.y + 72;
  ...
  renderWires(visibleNodeIds = null) {
    ...
    for (const edge of this.projection.edges || []) {
      ...
      const source = this.positions[sourceId], target = this.positions[targetId];
      if (!source || !target) continue;
      const x1 = source.x + 260, y1 = source.y + 72, x2 = target.x, y2 = target.y + 72;
      ...
      addReroute.setAttribute('cy', String((y1 + y2) / 2 + 22)); addReroute.setAttribute('r', '7');
  ...
  renderMinimap() {
    ...
    if (!points.length) { this.minimap.replaceChildren(); return; }
    const minX = Math.min(...points.map(point => point.x)) - 40, minY = Math.min(...points.map(point => point.y)) - 40;
    const maxX = Math.max(...points.map(point => point.x + 260)) + 40, maxY = Math.max(...points.map(point => point.y + 120)) + 40;
    this.minimap.setAttribute('viewBox', minX + ' ' + minY + ' ' + Math.max(1, maxX - minX) + ' ' + Math.max(1, maxY - minY));
    const fragment = document.createDocumentFragment();
    for (const [id, point] of Object.entries(this.positions)) {
      ...
      rect.setAttribute('width', '260'); rect.setAttribute('height', '100');
      rect.setAttribute('class', 'minimap-node' + (this.selection.has(id) ? ' selected' : '')); fragment.append(rect);
    }
    const viewport = document.createElementNS(SVG, 'rect');
    ...
    viewport.setAttribute('width', String(this.canvas.clientWidth / this.viewport.zoom));
    viewport.setAttribute('height', String(this.canvas.clientHeight / this.viewport.zoom));
    viewport.setAttribute('class', 'minimap-viewport'); fragment.append(viewport);
  ...
  startRerouteDrag(event, edgeId, index, target) {
    ...
    const move = current => { moved = true; point.x = initial.x + (current.clientX - start.x) / this.viewport.zoom;
      point.y = initial.y + (current.clientY - start.y) / this.viewport.zoom; this.schedule(); };
    const end = () => { target.removeEventListener('pointermove', move); if (moved) this.layoutCommand(before); };
  ...
  focusNode(id) {
    ...
    this.viewport.x = this.canvas.clientWidth / 2 - (point.x + 130) * this.viewport.zoom;
  ...
  startMarquee(event) {
    ...
    const end = current => {
      ...
      for (const [id, point] of Object.entries(this.positions))
        if (point.x + 260 >= left && point.x <= right && point.y + 120 >= top && point.y <= bottom) this.selection.add(id);
  ...
  startNodeDrag(event, nodeId) {
    ...
    const move = current => {
      ...
      for (const id of selected) this.positions[id] = { x: initial[id].x + dx, y: initial[id].y + dy };
      if (!committed) { committed = true; this.options.onCommand?.({ type: 'LAYOUT', before, after: this.snapshot() }); }
      this.schedule();
    };
    const end = () => { target.removeEventListener('pointermove', move); if (committed) this.changed(); };
  ...
  zoomToFit() {
    ...
    const maxX = Math.max(...points.map(point => point.x + 260)), maxY = Math.max(...points.map(point => point.y + 130));
    const zoom = Math.max(.2, Math.min(1.5, Math.min((this.canvas.clientWidth - 80) / Math.max(1, maxX - minX),
      (this.canvas.clientHeight - 80) / Math.max(1, maxY - minY))));
    this.viewport = { zoom, x: 40 - minX * zoom, y: 40 - minY * zoom };
  ...
  align(mode) {
    ...
    const value = mode === 'left' ? Math.min(...points.map(point => point.x))
      : mode === 'right' ? Math.max(...points.map(point => point.x + 260))
        : mode === 'top' ? Math.min(...points.map(point => point.y))
          : Math.max(...points.map(point => point.y + 120));
    for (const id of ids) {
      if (mode === 'left') this.positions[id].x = value;
      else if (mode === 'right') this.positions[id].x = value - 260;
      else if (mode === 'top') this.positions[id].y = value;
      else this.positions[id].y = value - 120;
    }
    this.schedule(); this.layoutCommand(before);
  }
  ...
  focusRelated(direction) {
    ...
    for (const edge of this.projection.edges || []) {
      const source = owners.get(edge.sourcePinId), target = owners.get(edge.targetPinId);
      if (direction === 'upstream' && this.selection.has(target)) related.add(source);
      if (direction === 'downstream' && this.selection.has(source)) related.add(target);
    }
    const values = [...related].filter(Boolean); if (!values.length) return;
    this.selection = new Set(values); this.schedule(); this.options.onSelection(this.projection.nodes.filter(node => this.selection.has(node.id)));
    if (values.length === 1) this.focusNode(values[0]); else this.zoomToFitSelection(values);
  }
  ...
  zoomToFitSelection(ids) {
    ...
    const zoom = Math.max(.2, Math.min(1.5, Math.min((this.canvas.clientWidth - 80) / Math.max(1, maxX - minX),
      (this.canvas.clientHeight - 80) / Math.max(1, maxY - minY))));
    this.viewport = { zoom, x: 40 - minX * zoom, y: 40 - minY * zoom }; this.applyViewport(); this.changed();
  }
  ...
  keydown(event) {
    ...
    if ((event.ctrlKey || event.metaKey) && ['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown'].includes(event.key)) {
      event.preventDefault(); const before = this.snapshot(), step = event.shiftKey ? 16 : 4;
      for (const id of this.selection) {
