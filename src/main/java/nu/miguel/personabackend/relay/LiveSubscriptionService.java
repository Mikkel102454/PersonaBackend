...
public final class LiveSubscriptionService {
    ...
    private static boolean allowedTopics(Set<String> topics,LiveStateSnapshot value){return (value.players().isEmpty()||topics.contains("PLAYERS"))&&(value.npcs().isEmpty()||topics.contains("NPCS"))&&(value.behaviors().isEmpty()||topics.contains("BEHAVIORS"))&&(value.quests().isEmpty()||topics.contains("QUESTS"))&&(value.dialogues().isEmpty()||topics.contains("DIALOGUES"))&&(value.memories().isEmpty()||topics.contains("MEMORIES"))&&(value.traces().isEmpty()||topics.contains("TRACES"))&&(value.server()==null||topics.contains("SERVER"));}
    private static int items(LiveStateSnapshot value){return value.players().size()+value.npcs().size()+value.behaviors().size()+value.quests().size()+value.dialogues().size()+value.memories().size()+value.traces().size()+value.removedKeys().size();}
    private static int filterSize(LiveFilter value){return value.playerIds().size()+value.npcDefinitions().size()+value.npcInstances().size()+value.worlds().size()+value.tracepoints().size()+value.watchedPins().size();}
